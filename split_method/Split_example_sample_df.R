...
id_based_split = function(data, Seed) {
  samples = lapply(split(data, data$id ), function(x) {
    set.seed(Seed)
    s = sample(seq_len(nrow(x)), size=floor(nrow(x)*0.75))
    x[s,]
  })
  ...
  # Example to check that train and test did not overlap
  # sort(as.numeric(row.names(samples$`101`)))
  # sort(as.numeric(row.names(samples$`101`)))
